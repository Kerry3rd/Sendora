import { 
  ...
  AzamPayService,
  AzamPayCredentials
 } from '../services/payment/azampay.service';
...
export const initializeAzamPay = (): AzamPayService | null => {
  const credentials: AzamPayCredentials = {
    appName: process.env.AZAMPAY_APP_NAME || 'SENDORA',
    clientId: process.env.AZAMPAY_CLIENT_ID || '',
    clientSecret: process.env.AZAMPAY_CLIENT_SECRET || '',
    apiKey: process.env.AZAMPAY_API_KEY || '',
    environment:
      (process.env.AZAMPAY_ENVIRONMENT as 'sandbox' | 'production') ||
      'sandbox'
  };
